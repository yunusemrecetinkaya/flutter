...
class _OnemliState extends State<Onemli> {
  ...
  List<String> gorevler = List();
  String temp;

  void listeyeEkle(String gelenGorev) async {
    final kayitAraci = await SharedPreferences.getInstance();

    if (formKey.currentState.validate()) {
      formKey.currentState.save();
      kayitAraci.setString("gorev", gelenGorev);
      var yazdirGorev = kayitAraci.getString("gorev");
      setState(() {
        gorevler.add(yazdirGorev);
        print(gorevler);
      });

      //Gorevler.withGorev(ekle.gorev);
      debugPrint("eklendi");
    }
  }
  ...
  Widget icerik() {
    return Padding(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(
            "Bu alanda yıldızladığınız görevler gözükür.",
